...
/// Persists and manages the Qwen-Image 2.1 generation queue.
@Observable
@MainActor
final class QwenImageJobStore {
    ...
    private static let jobsURL: URL = appSupportURL.appendingPathComponent("qwenimage-jobs.json")
    private static let maxJobs = 100

    var jobs: [QwenImageJob] = []
    var isRunning: Bool = false

    @ObservationIgnored private let saveDebouncer = Debouncer()

    var pendingJobs: [QwenImageJob] {
    ...
    func add(_ job: QwenImageJob) {
        jobs.insert(job, at: 0)
        pruneIfNeeded()
        save()
    }

    func addBatch(_ jobs: [QwenImageJob]) {
    ...
    func expandBatchJob(_ batchJob: QwenImageJob) {
    ...
    func cancelJob(_ job: QwenImageJob) {
    ...
    func restart(_ job: QwenImageJob) {
        job.status = .pending
        job.log = ""
        job.outputPath = nil
    ...
    private func load() {
        ...
        guard let loaded = try? decoder.decode([QwenImageJob].self, from: data) else { return }
        for job in loaded where job.status == .running {
            job.status = .failed("Interrupted: the app was quit during generation")
        }
        jobs = loaded
    }
}
