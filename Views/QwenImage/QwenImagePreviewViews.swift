...
/// Live stepwise preview for a running Qwen-Image job. Concrete copy of
/// ``StepwisePreviewView`` (the codebase keeps one per model family) bound to
/// ``QwenImageJob``.
struct QwenImageStepwisePreviewView: View {
...
/// Completed-image preview for a Qwen-Image job. Shows the image plus reveal/log
...
struct QwenImageCompletedPreviewView: View {
    let job: QwenImageJob
    var onRemix: ((QwenImageMetadata) -> Void)?
    var onApplySettings: ((QwenImageMetadata) -> Void)?
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            ImageMetadataPanel(
                info: ImageMetadataInfo(qwenImageJob: job),
    ...
    private var metadata: QwenImageMetadata {
        QwenImageMetadata.from(job: job)
    }
