...
///
/// The library is the single catalog of LoRAs, and ``LibraryLora/isDefault``
/// is the single source of the "default LoRAs" auto-applied to new
/// generations. A saved combo is a ``LoraStack``, built from these same
/// entries.
    ...
    private func row(_ entry: LibraryLora) -> some View {
        HStack(spacing: 8) {
            Toggle(isOn: Binding(
                ...
                set: { on in
                    var updated = entry
                    updated.isDefault = on
                    libraryStore.upsert(updated)
                }
            )) {
...
private struct LibraryLoraEditSheet: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            Toggle("Default LoRA — auto-apply to every new \(family.rawValue) generation", isOn: $draft.isDefault)
                .toggleStyle(.checkbox)
                .font(.caption)
        ...
        VStack(alignment: .leading, spacing: 8) {
            HStack {
                ...
                Menu {
                    ...
                    let defaults = libraryStore.defaultLoras(for: family)
