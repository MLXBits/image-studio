...
// MARK: - QwenImageQueueDrawerView

struct QwenImageQueueDrawerView: View {
    @Environment(QwenImageJobStore.self) private var store
    @Environment(QwenImageJobRunner.self) private var runner
    @Environment(AppSettings.self) private var settings
    @Environment(GenerationCoordinator.self) private var coordinator
    @Environment(TimingStore.self) private var timing

    @Binding var selectedJob: QwenImageJob?
    ...
    private func isRestartable(_ job: QwenImageJob) -> Bool {
        switch job.status {
        case .failed, .cancelled: true
        default: false
        }
    ...
    private func isCancellable(_ job: QwenImageJob) -> Bool {
        switch job.status {
        case .pending, .running: true
        default: false
        }
    ...
    private func restart(_ job: QwenImageJob) {
        store.restart(job)
        runner.runNext(in: store, settings: settings, coordinator: coordinator, timing: timing)
    }

    private func cancelJob(_ job: QwenImageJob) {
        if case .running = job.status {
            runner.cancel()
        } else {
            store.cancelJob(job)
...
// MARK: - QwenImageQueueJobRow

private struct QwenImageQueueJobRow: View {
