...
/// Z-Image submission form. Text-to-image (+ optional img2img init image):
/// prompt, dimensions, steps, seed/batch, LoRA, and output folder. The distilled
/// Turbo variant is guidance-free (no guidance slider, no negative prompt); the
/// base variant adds classifier-free guidance and a negative prompt.
struct ZImageParamsPanelView: View {
    ...
    /// Learned-time estimate for the current Z-Image configuration.
    ...
    var body: some View {
        SectionContainerView(
            title: "Prompt",
            info: params.isTurbo
                ? "Describe what you want to generate. Z-Image Turbo is distilled and runs "
                + "guidance-free for fast, single-pass generation.\n\n"
                ...
                : "Describe what you want to generate. Base Z-Image uses classifier-free "
                + "guidance; add a negative prompt to steer away from elements.\n\n"
                + "Wildcards: {red|black|white} makes Generate run one job per option "
                + "(up to 10), in order. Smaller groups cycle; a batch count overrides."
        ) {
            GrowingPromptField(
            ...
            if !params.isTurbo {
                GrowingPromptField(
                    ...
                    hint: "Describe elements to avoid"
                )
            }
        } accessory: {
            HStack(spacing: 6) {
        ...
        if !params.isTurbo {
            ...
            SectionContainerView(
                title: "Guidance",
                info: "How closely the model follows your prompt. Higher = stricter adherence "
                    + "but can over-saturate. 3–5 is typical. Z-Image Turbo runs guidance-free."
            ) {
                guidanceRow
            }
        }
        ...
        LoraManagerView(
            loras: $params.loras,
            showAdd: false,
            defaultLoras: loraLibrary.defaultLoras(for: .zimage),
            modelFamily: .zimage,
            library: loraLibrary,
            onInsertTriggerWords: { params.prompt = insertTriggerWords($0, into: params.prompt) },
            onReset: {
                params.loras = loraLibrary.defaultLoras(for: .zimage)
            }
        )
        .padding(.bottom, 8)
    }
            ...
            VStack(alignment: .leading, spacing: 4) {
                HStack(spacing: 3) {
                    ...
                    InfoButton(
                        ...
                        description: params.isTurbo
                            ? "Z-Image Turbo is distilled for ~8 steps. More steps add compute "
                            + "with diminishing returns; fewer may reduce quality."
                            : "Base Z-Image needs more steps than the Turbo variant — 30–50 is "
                            + "typical. More steps = more compute with diminishing quality returns."
                    )
                }
    ...
    private func pasteImage() {
        ...
        let tmp = FileManager.default.temporaryDirectory
            .appendingPathComponent("pasted-zimage-\(Int(Date().timeIntervalSince1970)).png")
